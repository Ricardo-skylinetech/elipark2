                ...
                <div class="container-fluid">
                    <!-- Page Heading -->
                    <h1 class="h3 mb-4 text-gray-800">Cátalogo Vales a Retornar</h1>
                    <div class="row">
                        <div class="col-lg-6">
                            <!-- DataTales Example -->
                            <div class="card shadow mb-4">
                                <div class="card-header py-3">
                                    <h6 class="m-0 font-weight-bold text-primary">Vales a Retornar</h6>
                                </div>
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-sm" id="tbl_cValesRetornar" width="100%" cellspacing="0">
                                            <thead>
                                                <tr>
                                                    <th>Id</th>
                                                    <th>Concepto</th>
                ...
                <script>
                    ...
                    $(document).ready(function() {
                        let tbl_cValesRetornar = $('#tbl_cValesRetornar').DataTable();
                        fn_tbl_cValesRetornar(estatus=1);

                        function fn_tbl_cValesRetornar(estatus){
                            tbl_cValesRetornar = $('#tbl_cValesRetornar').DataTable({
                                destroy: true,
                                dom: 'Bfrtip',
                                buttons: [
                                    // {
                                    ...
                                    //         (estatus == 1 ? fn_tbl_cValesRetornar(estatus=0) : fn_tbl_cValesRetornar(estatus=1));
                                    //     }
                                    // },
                                    {
                        ...
                        $("#tbl_cValesRetornar tbody").on("click", "a.estatus", function () {
                            let data = tbl_cValesRetornar.row($(this).parents("tr")).data();
                            let id = data.id;
                            let estatus = $(this).data('estatus');
                            console.log(id);
                            $('.loading').fadeIn('slow');
                            $.post("../../updateEstatusCatValesRetornar", {id:id,estatus:estatus},
                            function(result){
                                $('.loading').fadeOut('slow');
                                if(result.validacion){
                                    Swal.fire({
                                        ...
                                        tbl_cValesRetornar.ajax.reload();
                                    });
                                }else{
                                    Swal.fire({
                                        title:'Error',
                        ...
                        $("#frm-alta").submit(function(event){
                            ...
                            $('.loading').fadeIn('slow');
                            $.post('../../insertConceptoValesRetornar',data,
                            function(result){
                                $('.loading').fadeOut('slow');
                                if(result.validacion){
                                    Swal.fire({
                        ...
                        $("#tbl_cValesRetornar tbody").on("click", "a.editConcepto", function () {
                            let data = tbl_cValesRetornar.row($(this).parents("tr")).data();
                            $("#idC_e").val(data.id);
                        ...
                        $("#frm-editConcepto").submit(function(event) {
                            ...
                            var data = $(this).serialize();
                            $('.loading').fadeIn('slow');
                            $.post('../../updateConceptoValesRetornar', data,
                            function(result) {
                                $('.loading').fadeOut('slow');
                                if(result.validacion){
                                    Swal.fire({
