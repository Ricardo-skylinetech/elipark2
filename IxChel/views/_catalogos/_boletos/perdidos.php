                ...
                <div class="container-fluid">
                    <!-- Page Heading -->
                    <h1 class="h3 mb-4 text-gray-800">Cátalogo Boletos Perdidos</h1>
                    <div class="row">
                        <div class="col-lg-6">
                            <!-- DataTales Example -->
                            <div class="card shadow mb-4">
                                ...
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-sm" id="tbl_cPerdidos" width="100%" cellspacing="0">
                                            <thead>
                                                <tr>
                                                    <th>Id</th>
                                                    <th>Concepto</th>
                        ...
                        <div class="col-lg-6">
                            ...
                            <div class="card shadow mb-4">
                                <div class="card-header py-3">
                                    <h6 class="m-0 font-weight-bold text-primary">Tarifas Boletos Perdidos</h6>
                                </div>
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-sm tarifas" id="tbl_tPerdidos" width="100%" cellspacing="0">
                                            <thead>
                                                <tr>
                                                    <th>Id</th>
                                                    <th>Tarifa</th>
                ...
                <script>
                    ...
                    $(document).ready(function() {
                        let tbl_cPerdidos = $('#tbl_cPerdidos').DataTable();
                        fn_tbl_cPerdidos(estatus=1);

                        let tbl_tPerdidos = $('#tbl_tPerdidos').DataTable();
                        fn_tbl_tPerdidos(estatus=1);

                        function fn_tbl_cPerdidos(estatus){
                            tbl_cPerdidos = $('#tbl_cPerdidos').DataTable({
                                destroy: true,
                                dom: 'Bfrtip',
                                buttons: [
                                    // {
                                    ...
                                    //         (estatus == 1 ? fn_tbl_cPerdidos(estatus=0) : fn_tbl_cPerdidos(estatus=1));
                                    //     }
                                    // },
                                    {
                        ...
                        $("#tbl_cPerdidos tbody").on("click", "a.estatus", function () {
                            let data = tbl_cPerdidos.row($(this).parents("tr")).data();
                            let id = data.id;
                            let estatus = $(this).data('estatus');
                            
                            $('.loading').fadeIn('slow');
                            $.post("../../updateEstatusCatBperdidos", {id:id,estatus:estatus},
                            function(result){
                                $('.loading').fadeOut('slow');
                        ...
                        $("#frm-altaConcepto").submit(function(event){
                            ...
                            $('.loading').fadeIn('slow');
                            $.post('../../insertConceptoCatBperdidos',data,
                            function(result){
                                $('.loading').fadeOut('slow');
                                if(result.validacion){
                                    Swal.fire({
                        ...
                        $("#tbl_cPerdidos tbody").on("click", "a.editConcepto", function () {
                            let data = tbl_cPerdidos.row($(this).parents("tr")).data();
                            $("#idC_e").val(data.id);
                        ...
                        $("#frm-editConcepto").submit(function(event) {
                            ...
                            var data = $(this).serialize();
                            $('.loading').fadeIn('slow');
                            $.post('../../updateConceptoCatBperdidos', data,
                            function(result) {
                                $('.loading').fadeOut('slow');
                                if(result.validacion){
                                    Swal.fire({
                        ...
                        function fn_tbl_tPerdidos(estatus = 1) {
                            tbl_tPerdidos = $('#tbl_tPerdidos').DataTable({
                                destroy: true,
                                dom: 'Bfrtip',
                                buttons: [
                                    // {
                                    //     text: (estatus == 1 ? 'Eliminados' : 'Activos'),
                                    //     action: function ( e, dt, node, config ) {
                                    //         (estatus == 1 ? fn_tbl_tPerdidos(estatus=0) : fn_tbl_tPerdidos(estatus=1));
                                    //     }
                                    // },
                                    {
                                        text: '<i class="fas fa-plus"></i>',
                        ...
                        $("#tbl_tPerdidos tbody").on("click", "a.estatus", function () {
                            let data = tbl_tPerdidos.row($(this).parents("tr")).data();
                            let id = data.id;
                            let estatus = $(this).data('estatus');

                            $('.loading').fadeIn('slow');
                            $.post("../../updateEstatusCatTperdidos", {id:id,estatus:estatus},
                            function(result){
                                $('.loading').fadeOut('slow');
                                if(result.validacion){
                                    Swal.fire({
                        ...
                        $("#frm-altaTarifa").submit(function(event){
                            ...
                            var data = $(this).serialize();
                            $('.loading').fadeIn('slow');
                            $.post('../../insertTarifaCatTperdidos',data,
                            function(result){
                                $('.loading').fadeOut('slow');
                                if(result.validacion){
                                    Swal.fire({
                        ...
                        $("#tbl_tPerdidos tbody").on("click", "a.editTarifa", function () {
                            let data = tbl_tPerdidos.row($(this).parents("tr")).data();
                            $("#idT_e").val(data.id);
                        ...
                        $("#frm-editTarifa").submit(function(event) {
                            ...
                            var data = $(this).serialize();
                            $('.loading').fadeIn('slow');
                            $.post('../../updateTarifaCatTperdidos', data,
                            function(result) {
                                $('.loading').fadeOut('slow');
                                if(result.validacion){
                                    Swal.fire({
