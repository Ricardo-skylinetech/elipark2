                ...
                <div class="container-fluid">
                    <!-- Page Heading -->
                    <h1 class="h3 mb-4 text-gray-800">Cátalogo Creación de Vouchers</h1>
                    <div class="row">
                        <div class="col-lg-9">
                            <!-- DataTales Example -->
                            <div class="card shadow mb-4">
                                <div class="card-header py-3">
                                    <h6 class="m-0 font-weight-bold text-primary">Creación de Vouchers</h6>
                                </div>
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-sm" id="tbl_cBvouchers" width="100%" cellspacing="0">
                                            <thead>
                                                <tr>
                                                    <th>Id</th>
                                                    <th>Concepto</th>
                ...
                <script>
                    ...
                    $(document).ready(function() {
                        let tbl_cBvouchers = $('#tbl_cBvouchers').DataTable();
                        fn_tbl_cBvouchers(estatus=1);

                        function fn_tbl_cBvouchers(estatus) {
                            tbl_cBvouchers = $('#tbl_cBvouchers').DataTable({
                                destroy: true,
                                dom: 'Bfrtip',
                                buttons: [
                                    // {
                                    ...
                                    //         (estatus == 1 ? fn_tbl_cBvouchers(estatus=0) : fn_tbl_cBvouchers(estatus=1));
                                    //     }
                                    // },
                                    {
                        ...
                        $("#tbl_cBvouchers tbody").on("click", "a.estatus", function () {
                            let data = tbl_cBvouchers.row($(this).parents("tr")).data();
                            let id = data.id;
                            let estatus = $(this).data('estatus');
                            console.log(id);
                            $('.loading').fadeIn('slow');
                            $.post("../../updateEstatusCatBvouchers", {id:id,estatus:estatus},
                            function(result){
                                $('.loading').fadeOut('slow');
                        ...
                        $("#frm-alta").submit(function(event){
                            ...
                            $('.loading').fadeIn('slow');
                            $.post('../../insertConceptoBvouchers',data,
                            function(result){
                                $('.loading').fadeOut('slow');
                                if(result.validacion){
                                    Swal.fire({
                        ...
                        $("#tbl_cBvouchers tbody").on("click", "a.editConcepto", function () {
                            let data = tbl_cBvouchers.row($(this).parents("tr")).data();
                            $("#idC_e").val(data.id);
                        ...
                        $("#frm-editConcepto").submit(function(event) {
                            ...
                            $('.loading').fadeIn('slow');
                            $.post('../../updateConceptoBvouchers', data,
                            function(result) {
                                $('.loading').fadeOut('slow');
                                if(result.validacion){
                                    Swal.fire({
