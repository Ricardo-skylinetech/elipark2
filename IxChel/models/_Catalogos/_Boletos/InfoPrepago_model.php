...
class InfoPrepago_model extends CI_Model{

	protected $cat_estacionamientos = 'cat_boletos_info_prepago';
    ...
    public function getCatBinfoPrepago($estatus){
        ...
        $this->db->from("cat_boletos_info_prepago b");
    ...
    public function updateEstatusCatBinfoPrepago($id,$estatus){
        ...
        $result = $this->db->update('cat_boletos_info_prepago', $data, array('id'=>$id));

        $response = array("validacion" => ($result > 0 ? TRUE : FALSE));
		return $response;
    }

    public function insertConceptoBinfoPrepago($concepto,$estacionamiento){
        $data = array();
        ...
        foreach ($data as $row) {
            ...
            $query = $this->db->get('cat_boletos_info_prepago');
            if ($query->num_rows() > 0) {
        ...
        $this->db->trans_start();
        $this->db->insert_batch('cat_boletos_info_prepago', $data);
