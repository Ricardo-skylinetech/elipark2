...
class Operacion_model extends CI_Model{
	...
	protected $cat_estacionamientos = 'cat_boletos_operacion';
    ...
    public function getCatBoperacion($estatus){
        ...
        $this->db->from("cat_boletos_operacion b");
    ...
    public function updateEstatusCatBoperacion($id,$estatus){
        ...
        $result = $this->db->update('cat_boletos_operacion', $data, array('id'=>$id));

        $response = array("validacion" => ($result > 0 ? TRUE : FALSE));
		return $response;
    }

    public function insertConceptoBoperacion($concepto,$estacionamiento){
        $data = array();
        ...
        foreach ($data as $row) {
            ...
            $query = $this->db->get('cat_boletos_operacion');
            if ($query->num_rows() > 0) {
        ...
        $this->db->trans_start();
        $this->db->insert_batch('cat_boletos_operacion', $data);
