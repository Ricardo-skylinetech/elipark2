...
class ValesRetornar_model extends CI_Model{

	protected $cat_estacionamientos = 'cat_boletos_vales_retornar';
    ...
    public function getCatValesRetornar($estatus){
        ...
        $this->db->from("cat_boletos_vales_retornar b");
    ...
    public function updateEstatusCatValesRetornar($id,$estatus){
        ...
        $result = $this->db->update('cat_boletos_vales_retornar', $data, array('id'=>$id));

        $response = array("validacion" => ($result > 0 ? TRUE : FALSE));
		return $response;
    }

    public function insertConceptoValesRetornar($concepto,$estacionamiento){
        $data = array();
        ...
        foreach ($data as $row) {
            ...
            $query = $this->db->get('cat_boletos_vales_retornar');
            if ($query->num_rows() > 0) {
        ...
        $this->db->trans_start();
        $this->db->insert_batch('cat_boletos_vales_retornar', $data);
