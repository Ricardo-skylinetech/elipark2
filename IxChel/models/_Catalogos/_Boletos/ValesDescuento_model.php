...
class ValesDescuento_model extends CI_Model{
    ...
    public function getCatValesDescuento($estatus){
        ...
        $this->db->from("cat_boletos_vales_descuento b");
    ...
    public function updateEstatusCatValesDescuento($id,$estatus){
        ...
        $result = $this->db->update('cat_boletos_vales_descuento', $data, array('id'=>$id));

        $response = array("validacion" => ($result > 0 ? TRUE : FALSE));
		return $response;
    }

    public function insertConceptoCatValesDescuento($concepto,$estacionamiento){
        $data = array();
        ...
        foreach ($data as $row) {
            ...
            $query = $this->db->get('cat_boletos_vales_descuento');
            if ($query->num_rows() > 0) {
        ...
        $this->db->trans_start();
        $this->db->insert_batch('cat_boletos_vales_descuento', $data);
