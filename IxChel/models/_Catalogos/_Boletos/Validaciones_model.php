...
class Validaciones_model extends CI_Model{
	...
	protected $cat_estacionamientos = 'cat_boletos_validaciones';
    ...
    public function getCatBvalidaciones($estatus){
        ...
        $this->db->from("cat_boletos_validaciones b");
    ...
    public function updateEstatusCatBvalidaciones($id,$estatus){
        ...
        $result = $this->db->update('cat_boletos_validaciones', $data, array('id'=>$id));

        $response = array("validacion" => ($result > 0 ? TRUE : FALSE));
		return $response;
    }

    public function insertConceptoBvalidaciones($concepto,$estacionamiento){
        $data = array();
        ...
        foreach ($data as $row) {
            ...
            $query = $this->db->get('cat_boletos_validaciones');
            if ($query->num_rows() > 0) {
        ...
        $this->db->trans_start();
        $this->db->insert_batch('cat_boletos_validaciones', $data);
