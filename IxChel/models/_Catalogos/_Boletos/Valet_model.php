...
class Valet_model extends CI_Model{
	...
	protected $cat_boletos_valet = 'cat_boletos_valet';
    ...
    public function getCatBoletosValet($estatus){
        ...
        $this->db->from("cat_boletos_valet b");
    ...
    public function updateEstatusCatValet($id,$estatus){
        ...
        $result = $this->db->update('cat_boletos_valet', $data, array('id'=>$id));

        $response = array("validacion" => ($result > 0 ? TRUE : FALSE));
		return $response;
    }

    public function insertConceptoCatValet($concepto,$estacionamiento){
        $data = array();
        ...
        foreach ($data as $row) {
            ...
            $query = $this->db->get('cat_boletos_valet');
            if ($query->num_rows() > 0) {
        ...
        $this->db->trans_start();
        $this->db->insert_batch('cat_boletos_valet', $data);
