...
class Recobros_model extends CI_Model{
    ...
    public function getCatBrecobros($estatus){
        ...
        $this->db->from("cat_boletos_recobros b");
    ...
    public function getCatTrecobros($estatus){
        $estacionamiento = $this->session->userdata('id_estacionamiento');
        ...
        $this->db->from("cat_tarifas_boletos_recobros t");
    ...
    public function updateEstatusCatBrecobros($id,$estatus){
        ...
        $result = $this->db->update('cat_boletos_recobros', $data, array('id'=>$id));

        $response = array("validacion" => ($result > 0 ? TRUE : FALSE));
		return $response;
    }

    public function updateEstatusCatTrecobros($id,$estatus){
        ...
        $result = $this->db->update('cat_tarifas_boletos_recobros', $data, array('id'=>$id));

        $response = array("validacion" => ($result > 0 ? TRUE : FALSE));
		return $response;
    }

    public function insertConceptoCatBrecobros($concepto,$estacionamiento){
        $data = array();
        ...
        foreach ($data as $row) {
            ...
            $query = $this->db->get('cat_boletos_recobros');
            if ($query->num_rows() > 0) {
        ...
        $this->db->trans_start();
        $this->db->insert_batch('cat_boletos_recobros', $data);
    ...
    public function insertTarifaCatTrecobros($tarifa,$estacionamiento){
        $data = array();
        ...
        foreach ($data as $row) {
            ...
            $this->db->where('estacionamiento_id', $row['estacionamiento_id']);
            $query = $this->db->get('cat_tarifas_boletos_recobros');
            if ($query->num_rows() > 0) {
        ...
        // Realizar la inserción
        $this->db->trans_start();
        $this->db->insert_batch('cat_tarifas_boletos_recobros', $data);
        $this->db->trans_complete();
