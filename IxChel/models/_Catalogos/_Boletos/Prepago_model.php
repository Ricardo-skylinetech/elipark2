...
class Prepago_model extends CI_Model{
    ...
    public function getCatBprepago($estatus){
        ...
        $this->db->from("cat_boletos_prepago b");
    ...
    public function getCatTprepago($estatus){
        $estacionamiento = $this->session->userdata('id_estacionamiento');
        ...
        $this->db->from("cat_tarifas_boletos_prepago t");
    ...
    public function updateEstatusCatBprepago($id,$estatus){
        ...
        $result = $this->db->update('cat_boletos_prepago', $data, array('id'=>$id));

        $response = array("validacion" => ($result > 0 ? TRUE : FALSE));
		return $response;
    }

    public function updateEstatusCatTprepago($id,$estatus){
        ...
        $result = $this->db->update('cat_tarifas_boletos_prepago', $data, array('id'=>$id));

        $response = array("validacion" => ($result > 0 ? TRUE : FALSE));
		return $response;
    }

    public function insertConceptoCatBprepago($concepto,$estacionamiento){
        $data = array();
        ...
        foreach ($data as $row) {
            ...
            $query = $this->db->get('cat_boletos_prepago');
            if ($query->num_rows() > 0) {
        ...
        $this->db->trans_start();
        $this->db->insert_batch('cat_boletos_prepago', $data);
    ...
    public function insertTarifaCatTprepago($tarifa,$estacionamiento){
        $data = array();
        ...
        foreach ($data as $row) {
            ...
            $this->db->where('estacionamiento_id', $row['estacionamiento_id']);
            $query = $this->db->get('cat_tarifas_boletos_prepago');
            if ($query->num_rows() > 0) {
        ...
        // Realizar la inserción
        $this->db->trans_start();
        $this->db->insert_batch('cat_tarifas_boletos_prepago', $data);
        $this->db->trans_complete();
