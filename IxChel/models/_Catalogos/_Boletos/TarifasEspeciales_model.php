...
class TarifasEspeciales_model extends CI_Model{
    ...
    public function getCatBtarifasEspeciales($estatus){
        ...
        $this->db->from("cat_boletos_tarifas_especiales b");
    ...
    public function getCatTtarifasEspeciales($estatus){
        $estacionamiento = $this->session->userdata('id_estacionamiento');
        ...
        $this->db->from("cat_tarifas_boletos_tarifas_especiales t");
    ...
    public function updateEstatusCatTarifasEspeciales($id,$estatus){
        ...
        $result = $this->db->update('cat_boletos_tarifas_especiales', $data, array('id'=>$id));

        $response = array("validacion" => ($result > 0 ? TRUE : FALSE));
		return $response;
    }

    public function updateEstatusCatTtarifasEspeciales($id,$estatus){
        ...
        $result = $this->db->update('cat_tarifas_boletos_tarifas_especiales', $data, array('id'=>$id));

        $response = array("validacion" => ($result > 0 ? TRUE : FALSE));
		return $response;
    }

    public function insertConceptoCatTarifasEspeciales($concepto,$estacionamiento){
        $data = array();
        ...
        foreach ($data as $row) {
            ...
            $query = $this->db->get('cat_boletos_tarifas_especiales');
            if ($query->num_rows() > 0) {
        ...
        $this->db->trans_start();
        $this->db->insert_batch('cat_boletos_tarifas_especiales', $data);
    ...
    public function insertTarifaCatTTarifasEspeciales($tarifa,$estacionamiento){
        $data = array();
        ...
        foreach ($data as $row) {
            ...
            $this->db->where('estacionamiento_id', $row['estacionamiento_id']);
            $query = $this->db->get('cat_tarifas_boletos_tarifas_especiales');
            if ($query->num_rows() > 0) {
        ...
        // Realizar la inserción
        $this->db->trans_start();
        $this->db->insert_batch('cat_tarifas_boletos_tarifas_especiales', $data);
        $this->db->trans_complete();
