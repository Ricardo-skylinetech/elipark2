...
class Expedidos_model extends CI_Model{
    ...
    public function getCatBexpedidos($estatus){
        ...
        $this->db->from("cat_boletos_expedidos b");
    ...
    public function updateEstatusCatBexpedidos($id,$estatus){
        ...
        $result = $this->db->update('cat_boletos_expedidos', $data, array('id'=>$id));

        $response = array("validacion" => ($result > 0 ? TRUE : FALSE));
		return $response;
    }

    public function insertConceptoCatBexpedidos($concepto,$estacionamiento){
        $data = array();
        ...
        foreach ($data as $row) {
            ...
            $query = $this->db->get('cat_boletos_expedidos');
            if ($query->num_rows() > 0) {
        ...
        $this->db->trans_start();
        $this->db->insert_batch('cat_boletos_expedidos', $data);
