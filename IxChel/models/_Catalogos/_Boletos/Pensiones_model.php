...
class Pensiones_model extends CI_Model{
    ...
    public function getCatBpensiones($estatus){
        ...
        $this->db->from("cat_boletos_pensiones b");
    ...
    public function getCatTpensiones($estatus){
        $estacionamiento = $this->session->userdata('id_estacionamiento');
        ...
        $this->db->from("cat_tarifas_boletos_pensiones t");
    ...
    public function updateEstatusCatBpensiones($id,$estatus){
        ...
        $result = $this->db->update('cat_boletos_pensiones', $data, array('id'=>$id));

        $response = array("validacion" => ($result > 0 ? TRUE : FALSE));
		return $response;
    }

    public function updateEstatusCatTpensiones($id,$estatus){
        ...
        $result = $this->db->update('cat_tarifas_boletos_pensiones', $data, array('id'=>$id));

        $response = array("validacion" => ($result > 0 ? TRUE : FALSE));
		return $response;
    }

    public function insertConceptoCatBpensiones($concepto,$estacionamiento){
        $data = array();
        ...
        foreach ($data as $row) {
            ...
            $query = $this->db->get('cat_boletos_pensiones');
            if ($query->num_rows() > 0) {
        ...
        $this->db->trans_start();
        $this->db->insert_batch('cat_boletos_pensiones', $data);
    ...
    public function insertTarifaCatTpensiones($tarifa,$estacionamiento){
        $data = array();
        ...
        foreach ($data as $row) {
            ...
            $this->db->where('estacionamiento_id', $row['estacionamiento_id']);
            $query = $this->db->get('cat_tarifas_boletos_pensiones');
            if ($query->num_rows() > 0) {
        ...
        // Realizar la inserción
        $this->db->trans_start();
        $this->db->insert_batch('cat_tarifas_boletos_pensiones', $data);
        $this->db->trans_complete();
