...
class Cobrados_model extends CI_Model{
    ...
    public function getCatBcobrados($estatus){
        ...
        $this->db->from("cat_boletos_cobrados b");
    ...
    public function getCatTcobrados($estatus){
        $estacionamiento = $this->session->userdata('id_estacionamiento');
        ...
        $this->db->from("cat_tarifas_boletos_cobrados t");
    ...
    public function updateEstatusCatBcobrados($id,$estatus){
        ...
        $result = $this->db->update('cat_boletos_cobrados', $data, array('id'=>$id));

        $response = array("validacion" => ($result > 0 ? TRUE : FALSE));
		return $response;
    }

    public function updateEstatusCatTcobrados($id,$estatus){
        ...
        $result = $this->db->update('cat_tarifas_boletos_cobrados', $data, array('id'=>$id));

        $response = array("validacion" => ($result > 0 ? TRUE : FALSE));
		return $response;
    }

    public function insertConceptoCatBcobrados($concepto,$estacionamiento){
        $data = array();
        ...
        foreach ($data as $row) {
            ...
            $query = $this->db->get('cat_boletos_cobrados');
            if ($query->num_rows() > 0) {
        ...
        $this->db->trans_start();
        $this->db->insert_batch('cat_boletos_cobrados', $data);
    ...
    public function insertTarifaCatTcobrados($tarifa, $estacionamiento)
    {
        $data = array();
        ...
        foreach ($data as $row) {
            ...
            $this->db->where('estacionamiento_id', $row['estacionamiento_id']);
            $query = $this->db->get('cat_tarifas_boletos_cobrados');
            if ($query->num_rows() > 0) {
        ...
        // Realizar la inserción
        $this->db->trans_start();
        $this->db->insert_batch('cat_tarifas_boletos_cobrados', $data);
        $this->db->trans_complete();
