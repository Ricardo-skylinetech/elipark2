...
class Pases_model extends CI_Model{
    ...
    public function getCatBpases($estatus){
        ...
        $this->db->from("cat_boletos_pases b");
    ...
    public function getCatTPases($estatus){
        $estacionamiento = $this->session->userdata('id_estacionamiento');
        ...
        $this->db->from("cat_tarifas_boletos_pases t");
    ...
    public function updateEstatusCatTrecobros($id,$estatus){
        ...
        $result = $this->db->update('cat_tarifas_boletos_pases', $data, array('id'=>$id));
    ...
    public function insertConceptoCatBrecobros($concepto,$estacionamiento){
        ...
        foreach ($data as $row) {
            ...
            $query = $this->db->get('cat_boletos_pases');
            if ($query->num_rows() > 0) {
        ...
        $this->db->trans_start();
        $this->db->insert_batch('cat_boletos_pases', $data);
    ...
    public function insertTarifaCatTrecobros($tarifa,$estacionamiento){
        ...
        foreach ($data as $row) {
            ...
            $this->db->where('estacionamiento_id', $row['estacionamiento_id']);
            $query = $this->db->get('cat_tarifas_boletos_pases');
            if ($query->num_rows() > 0) {
        ...
        // Realizar la inserción
        $this->db->trans_start();
        $this->db->insert_batch('cat_tarifas_boletos_pases', $data);
        $this->db->trans_complete();
    ...
    public function updateConcepto($id,$concepto,$estacionamiento_id){
        ...
        $result = $this->db->update('cat_boletos_pases', $data, array('id'=>$id));
