...
class SinCobro_model extends CI_Model{

	protected $cat_estacionamientos = 'cat_boletos_sin_cobro';
    ...
    public function getCatBsinCobro($estatus){
        ...
        $this->db->from("cat_boletos_sin_cobro b");
    ...
    public function updateEstatusCatBsinCobro($id,$estatus){
        ...
        $result = $this->db->update('cat_boletos_sin_cobro', $data, array('id'=>$id));

        $response = array("validacion" => ($result > 0 ? TRUE : FALSE));
		return $response;
    }

    public function insertConceptoBsinCobro($concepto,$estacionamiento){
        $data = array();
        ...
        foreach ($data as $row) {
            ...
            $query = $this->db->get('cat_boletos_sin_cobro');
            if ($query->num_rows() > 0) {
        ...
        $this->db->trans_start();
        $this->db->insert_batch('cat_boletos_sin_cobro', $data);
